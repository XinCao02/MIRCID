...
## Paper Abstract
Drug mechanism-of-action (MoA) modeling commonly relies on perturbational transcriptomes, but endpoint gene expression can be noisy and may incompletely represent regulatory state. Here, we present MIRCID, a framework that systematically compares gene expression with inferred transcription factor (TF) activity and microRNA (miRNA) expression across pathway classification and similarity-based MoA retrieval. We developed HubmiRNet to infer 414 pan-cancer hub miRNAs (HubmiRs) from 977 L1000 landmark genes, achieving a Pearson correlation coefficient of 87.72%; its 1,298-output variant also outperformed SiCmiR on the original full-miRNA task (PCC 71.21% versus 67.30%). Across downstream models and tasks, miRNA augmentation yielded the most consistent and transferable gains, whereas TF activity provided limited incremental improvement. Dimension-matched generic embeddings did not reproduce the benefit of Gene+HubmiR, and complementarity analyses showed that HubmiR re- tained gene-derived structure while reorganizing it into a distinct, partially linearly recoverable feature space. Rescue analyses across various pathways further showed that the advantage of the miRNA layer was most evident when transcriptional signatures were weak, suggesting complementary post-transcriptional information being captured. These results identify inferred HubmiRs as a biologically informed, task-relevant recoding of transcriptomic data that improves perturbational drug modeling across classification and similarity-based retrieval.
